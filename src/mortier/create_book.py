...
for i in range(20):
    ...
    angle = np.random.uniform(low=writer.bands_width, high=np.pi/2 - writer.bands_width)
    if np.random.random() < 0.2:
        ...
        if np.random.random() < 0.2:
            writer.n_tiles = 2
            tesselation.set_assym_angle(angle + np.random.uniform(low=-angle/4, high=angle/4))
            writer.lacing_mode = False
            writer.bands_mode = False 
