...
class TikzWriter(Writer):
    ...
    def line(self, p0, p1, dotted = False, color = "black"):
        ...
        if self.in_bounds(p0) and self.in_bounds(p1):
            self.output.append(f"\\draw [draw={color}{pattern}] ({np.round(p0.x, 2)}, {np.round(p0.y, 2)}) -- ({np.round(p1.x, 2)}, {np.round(p1.y, 2)});")
    ...
    def face(self, face, dotted = False, outline = False):
        ...
        for p in face.mid_points:
            if str(p) not in self.intersect_points:
                ...
                self.intersect_points[str(p)] = np.array([(x + 1) % 2 for x in self.intersect_points[str(p)]])
    ...
    def set_bounds(self, size):
        if self.draw_borders:
            self.header += "\\draw (0, 0) rectangle + (" + str(size[2] - 2) + "," + str(size[3] - 2)+");\n"
        self.header += "\\clip ("+ str(size[0] + 1) + ","+ str(size[1] + 1) + ")"
        self.header += "rectangle + (" + str(size[2] - 2) + "," + str(size[3] - 2)+");\n"
